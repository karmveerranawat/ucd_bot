...
module.exports = {
	name: 'disable',
	description: 'disabling commands',
	execute(message, args) {

        if(status(args)){
            message.channel.send(`${message.author} , Command is already disabled! <:ucd_cringe:849590578249203742>`);
        } else{
            cmd = args[0]
            var temp_status = JSON.parse(fs.readFileSync("commands_status.json", 'utf-8'))
            temp_status[cmd] = true;
            fs.writeFileSync("commands_status.json", JSON.stringify(temp_status));
            message.channel.send(`${message.author} , Command has been disabled now! <:ucd_stinks:845339515135721534>`);
        }
