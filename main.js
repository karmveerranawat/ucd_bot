...
client.on('message', message => {
    ...
    if(command === 'ping'){
        if (!status(command)) return message.channel.send(`<:ucd_rage:872884631328948234> This command is disabled in server, ${message.author}! `);
        client.commands.get('ping').execute(message, args);
    } 

    else if(command === 'leave-reason'){
        if (!args.length) return message.channel.send(`Please provide a reason for your leave, ${message.author}!`);
        client.commands.get('leave-reason').execute(message, args);
    } 

    else if(command === 'count' ||command === 'c'){
        temp_cmds="count"
        if(status(temp_cmds)) return message.channel.send(`<:ucd_rage:872884631328948234> This command is disabled in server, ${message.author}! `);
        client.commands.get('count').execute(message, args);
        
    }

    else if(command === 'ref'){
        if(status(command) == true) return message.channel.send(`<:ucd_rage:872884631328948234> This command is disabled in server, ${message.author}! `);
        ...
        if (!args.length) return message.channel.send(`<:ucd_saste_nashe:845329317071683644> Atleast specify what to disable, ${message.author}! `);
